      ...
      return this.trigger("unmask").each(function () {
        ...
        input
          ...
          .on("input.mask paste.mask", function () {
            ...
            if (input.attr("data-tilda-mask").indexOf("+7") === 0) {
...
function t_form_dict(msg) {
  ...
  dict["successorder"] = {
    ...
    DE: "Vielen Dank! Bestellung erstellt. Bitte warten Sie, Sie werden zur Zahlungsseite weitergeleitet...",
  ...
  $(document).ready(function () {
    ...
    window.tildaForm.validate = function ($jform) {
          ...
          switch (rule) {
            ...
            case "phone":
              ...
              if (valnospace.length > 0 && !valnospace.match(regExp)) {
                ...
                if (
                  str.indexOf("000") == 1 ||
                  str.indexOf("111") == 1 ||
                  (str.indexOf("222") == 1 && str.substring(0, 1) != "5") ||
                  str.indexOf("333") == 1 ||
                  str.indexOf("444") == 1 ||
                  (str.indexOf("555") == 1 && str.substring(0, 1) != "0") ||
                  (str.indexOf("666") == 1 && str.substring(0, 1) != "0") ||
                  (str.indexOf("8888") == 1 && str.substring(0, 1) != "4")
                ) {
                  error.type.push("phone");
                }
              }
            ...
            case "date":
              ...
              if (
                valnospace.length > 0 &&
                !valnospace.match(
                  format[$(this).data("tilda-dateformat")] ||
            ...
            case "chosevalue":
              var isOptionSelected =
                $(this).attr("data-option-selected") === "true" ? true : false;
              if (!isOptionSelected) {
            ...
            case "promocode":
              if (
                $jform.attr("data-formcart") === "y" &&
                valnospace.length > 0 &&
                window.tcart &&
                (!window.tcart.promocode ||
                  !window.tcart.prodamount_discountsum)
              ) {
                error.type.push("promocode");
              }
              break;
            case "deliveryreq":
      ...
      if ($jform.attr("data-formcart") === "y") {
        var minOrderSetted =
          typeof window.tcart_minorder != "undefined" &&
          window.tcart_minorder > 0;
        var minQuantitySetted =
          typeof window.tcart_mincntorder != "undefined" &&
          window.tcart_mincntorder > 0;
        if (minOrderSetted) {
    ...
    window.tildaForm.showErrorInPopup = function ($jform, arErrors) {
      ...
      for (var i = 0; i < arErrors.length; i++) {
        ...
        $fieldgroup = arErrors[i].obj
          .closest(clsInputBoxSelector)
    ...
    window.tildaForm.addTildaCaptcha = function ($jform, formskey) {
      ...
      try {
        randomkey =
          "" + new Date().getTime() + "=" + parseInt(Math.random() * 8);
      } catch (e) {
    ...
    window.tildaForm.addMebersInfoToForm = function ($jform) {
      try {
        window.tildaForm.tildamember = {};
        $jform.find(".js-tilda-mauserinfo").remove();
    ...
    window.tildaForm.addPaymentInfoToForm = function ($jform) {
      ...
      if (
        ...
        $(".t-radio_delivery:checked").data("delivery-price") > 0
      ) {
        try {
      ...
      if (window.tcart.promocode && window.tcart.promocode.promocode > "") {
        ...
        if (
          window.tcart.prodamount_discountsum &&
          parseFloat(window.tcart.prodamount_discountsum) > 0
        ) {
        ...
        if (
          ...
          parseFloat(window.tcart.prodamount_withdiscount) > 0
        ) {
        ...
        if (
          ...
          parseFloat(window.tcart.amount_withoutdiscount) > 0
        ) {
          ...
          if (typeof product[j] != "function") {
            if (j == "options") {
              ...
              for (var o in product[j]) {
                if (
                  typeof window.tildaForm.tildapayment["products"][i][j][o] ==
                  "undefined"
                ) {
                  window.tildaForm.tildapayment["products"][i][j][o] = {};
                }
                if (product[j][o]["option"]) {
                  window.tildaForm.tildapayment["products"][i][j][o]["option"] =
                    product[j][o]["option"];
                }
                if (product[j][o]["price"] && product[j][o]["price"] > 0) {
                  window.tildaForm.tildapayment["products"][i][j][o]["price"] =
                    product[j][o]["price"];
                }
    ...
    window.tildaForm.paysystemScriptLoad = function (src, sysname) {
      ...
      if (
        ...
        window.scriptSysPayment[sysname] !== true
      ) {
    ...
    window.tildaForm.stripePay = function ($jform, arArgs) {
      ...
      if (!window.stripehandler) {
        ...
        if (arArgs.zipCode && arArgs.zipCode == 1) {
          arStripeInit.zipCode = true;
        }
        if (arArgs.billingAddress && arArgs.billingAddress == 1) {
          arStripeInit.billingAddress = true;
        }
        if (arArgs.shipping && arArgs.shipping == 1) {
          arStripeInit.shippingAddress = true;
        }
          ...
          if (token && token.id) {
            $.ajax({
              ...
              data: {
                ...
                currency: arArgs.currency,
                amount: parseInt(
                  (parseFloat(arArgs.amount) * multiple).toFixed()
                ),
              },
              dataType: "json",
              ...
              success: function (json) {
                ...
                if (
                  window.Tilda &&
                  typeof Tilda.sendEventToStatistics == "function"
                ) {
                  var currency =
                    $("#allrecords").data("tilda-currency") ||
                    $(".t706").data("project-currency-code");
    ...
    window.tildaForm.cloudpaymentPay = function ($jform, arArgs) {
      ...
      if (
        arArgs.cloudPayments &&
        (arArgs.cloudPayments.recurrent || arArgs.cloudPayments.customerReceipt)
      ) {
    ...
    window.tildaForm.sendStatAndShowMessage = function (
      ...
      if (
        successcallback &&
        successcallback.length > 0 &&
        successcallback === "t396_onSuccess" &&
        typeof window["t396_onSuccess"] === "function"
      ) {
    ...
    window.tildaForm.banktransferPay = function ($jform, arArgs) {
      if (arArgs && arArgs.condition == "fast") {
        window.tildaForm.sendStatAndShowMessage($jform, arArgs, true);
      } else {
    ...
    window.tildaForm.send = function (
      ...
      window.tildaForm.tildapayment = false;
      if (
        $jform.data("formcart") == "y" ||
        $jform.closest(".t706__orderform").length > 0
      ) {
        window.tildaForm.addPaymentInfoToForm($jform);
      }
      try {
        if (window.mauser) {
          window.tildaForm.addMebersInfoToForm($jform);
        }
      } catch (e) {}
      if (formtype == 2 || (!formtype && formskey > "")) {
        ...
        if ($inputElem.length > 0) {
          $inputElem.val(window.browserLang);
        }
            ...
            if (json && json.error) {
              ...
              if (!virtPage || virtPage == "") {
                if (
                  ...
                  ((json.next &&
                    json.next.type &&
                    (json.next.type != "function" ||
                      (json.next.value &&
                        (json.next.value.sysname == "stripev3" ||
                          json.next.value.installation == "outersite")))) ||
                    !json.next)
                ) {
    ...
    $(".r").on("click", ".js-form-proccess [type=submit]", function (event) {
      ...
      if (window.tildaForm.showErrors($activeForm, arErrors)) {
        ...
        if (
          $activeForm.find(".js-formaction-services").length == 0 &&
          formtype != 1 &&
          formskey == ""
        ) {
